...
public class GridPanelPeer extends AbstractComponentSynchronizePeer {
	...
	public GridPanelPeer() {
		...
		addEvent(new AbstractComponentSynchronizePeer.EventPeer(
				GridPanel.COLUMN_FILTER_ACTION, GridPanel.COLUMN_LISTENERS,
				FilterColumnEvent.class) {
			@Override
			public boolean hasListeners(Context context, Component component) {
				return (((GridPanel) component).hasColumnListeners());
			}
	...
	public void storeInputProperty(Context context, Component component,
		...
		if (PROPERTY_SELECTION.equals(propertyName)) {
			...
			for (; x < clientModel.getColumnCount(); x++) {
				...
				ColumnConfiguration serverColumn = serverModel
						.getColumn(ignoreFirstCol ? x - 1 : x);
				...
				serverColumn.setGroupableValues(clientColumn
						.getGroupableValues());
				serverColumn.setSelectedGroupableValues(clientColumn
						.getSelectedGroupableValues());
			...
			for (int row = 0; row < tma.getData().length; row++) {
				for (int col = 0; col < tma.getData()[row].length; col++) {
					String renderedValue = p.getGridCellRenderer()
							.getModelValue(p,
									currentModel.getValueAt(col, row + offset),
									col, row + offset);
					...
					if (renderedValue == null && newTMValue == null) {
						...
						currentModel.setValueAt(convertToType(newTMValue, cm
								.getColumn(col).getColumnClass()), col, row
								+ offset);
					} else {
						if (!renderedValue.equals(newTMValue))
							currentModel.setValueAt(convertToType(newTMValue,
									cm.getColumn(col).getColumnClass()), col,
									row + offset);
					}
				}
			}
	...
	private Object convertToType(Object value, Class type) {
		...
		if (String.class.equals(type)) {
			...
			if (value instanceof String) {
				try {
					return type.getMethod("valueOf",
							new Class[] { String.class }).invoke(null, value);
				} catch (IllegalArgumentException e) {
					throw new IllegalArgumentException(
							"Can't convert value to Number", e);
		...
		throw new IllegalArgumentException("I can't convert " + value + " to "
